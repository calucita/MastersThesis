...
void simd(float * __restrict__ A, const long N, const long M)
{
    ...
    for (long j = 0; j < M; j++) { 
		...
		#pragma omp simd simdlen(32) aligned(A,B)
		for (int i = 0; i < N/2; i++) {
			sum+=gather(A, B[i]);
		}
        if (N > 1000000000) A[0] = sum; // Avoid compiler's dead code detection.
...
inline __m256 gather(float *A, __m256i indices)
{
   ...
   // return _mm256_load_ps(&A[i[0]]);
    ...
    return _mm256_i32gather_ps(A, indices, 4);
}
...
int main(int argc, char *argv[])
{
    ...
    double t4 = timeit_mt(easy, A, N, M, 1,  MAX_ITERS);
    printf("Easy runtime [ms]:              %7.2lf\n", 1000 * t4);
    double t2 = timeit_mt(intrinsic3, A, N, M, 1,  MAX_ITERS);
