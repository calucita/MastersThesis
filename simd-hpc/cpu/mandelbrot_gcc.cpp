...
//	long pagesize = sysconf(_SC_PAGESIZE);
  ...
  //  float *C = (float*) _mm_malloc(N * M * sizeof(float), pagesize);
    ...
    double t1 = timeit(simd, B, N, M, 1, 1000);
    printf("SIMD kernel runtime:      %lf s\n", t1);
    if (DEBUG) printf("norm=%lf\n", norm(B, N, M));
    if (DEBUG) save(B, N, M, save_num++);

   // timeit(init, C, N, M, 1, 1);
/*    double t2 = timeit(intrinsic, C, N, M, 1, 1000);
    printf("Intrinsic kernel runtime: %lf s\n", t2);
    if (DEBUG) printf("norm=%lf\n", norm(C, N, M));
    if (DEBUG) save(C, N, M, save_num++);*/
   
    int * __restrict__ a = (int *) A;
    int * __restrict__ b = (int *) B;
  //  int * __restrict__ c = (int *) C;
	for (long i=0; i<N; i++){
	    for (long j=0; j<M; j++){
		if ((a[i*N+j]!=b[i*N+j])){
//			printf("%d %d \n", i,j);
