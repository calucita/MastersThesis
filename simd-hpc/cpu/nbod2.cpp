...
//    printf("Propagating particles using %d threads...\n", omp_get_max_threads());
    ...
    // Start timing
    #pragma omp parallel for simd simdlen(16) //schedule(dynamic) // simdlen(8) //schedule(auto)
    for (int i = 0; i < nParticles; i++) {
        ...
        // Vectorized loop over particles that exert force
            if (j != i) {
                ...
                float drSquared    = dx*dx + dy*dy + dz*dz;
...
//    printf("Propagating particles using %d threads...\n", omp_get_max_threads());
   ...
   #pragma nounroll_and_jam // To avoid loop interchange.
    ...
    // Components of force on particle 
      ...
      for(int j = 0; j < nParticles; j++) {
	if (j != i){
         __assume_aligned(p.x, 64);
     __assume_aligned(p.y, 64);
     __assume_aligned(p.z, 64);
 // Vectorized loop over particles that exert force
            // Newton’s law of universal gravity calculation.
            const float dx = p.x[j] - p.x[i]; 
            const float dy = p.y[j] - p.y[i];
    ...
    __assume_aligned(p.vx, 64);
     __assume_aligned(p.vy, 64);
     __assume_aligned(p.vz, 64);
    ...
    #pragma simd simdlen(32) //schedule(dynamic) simdlen(32)
    for(int i = 0; i < nParticles; i++) {
     __assume_aligned(p.x, 64);
     __assume_aligned(p.y, 64);
     __assume_aligned(p.z, 64);
...
int main(int argc, char **argv) {
    ...
    long N, M;
    cmdline_Bytes_nbod(argc, argv, /*16*3840 */ 6*7680*sizeof(float), 35 , &N, &M);
